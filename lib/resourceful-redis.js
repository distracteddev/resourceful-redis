...
function stringifyAllProps (obj) {
 for (var key in obj) {
...
Redis.prototype.save = function(key, val, cb) {
  ...
  async.waterfall([
    ...
    function write_hash(callback) {
      ...
      val = stringifyAllProps(val);
      //console.log("VAL:", val);

      self.connection.HMSET(id, val, function(err) {
        if(err) return callback(err);
        self.cache.put(key, parseAllProps(val));
        return callback(null);
      });
    },
...
Redis.prototype.update = function(key, val, cb) {
  ...
  this.get(key, function(err, old) {
    ...
    var placeholder = resourceful.mixin(old, val);
    placeholder = stringifyAllProps(placeholder);
    //console.log("placeholder", placeholder);
    self.connection.HMSET(id, placeholder, function(err) {
      if(err) return cb(err);
      self.cache.update(key, parseAllProps(val));
      self.get(key, cb);
    });
  });
};
...
Redis.prototype.find = function(conditions, cb) {
  ...
  this.filter(function(obj) {
    return Object.keys(conditions).every(function (k) {
      //console.log("COND: ",conditions, "OBJ: ",obj,"K: ", k);
  ...
  async.waterfall([
    ...
    function parse_results(callback) {
      result.forEach(function(data, idx) {
        result[idx] = data = parseAllProps(data);
